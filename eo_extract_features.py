...
def run_model(
    ...
    # mask_ratio: float,
    device: torch.device,
    save_path: str = "extracted_q.pt" # ADD this line
    ...
    "DELETE below till the end of function"
...
def main(
    ...
    os.makedirs("q_patch", exist_ok=True)

    for i, x in enumerate(windows): # i（インデックス）を追加
        # 保存名にインデックスを付けて、パッチごとのQを区別できるようにするとベター
        save_path = f"q_patch/extracted_q_patch_{i}.pt" 
        
        run_model(model, x, temporal_coords, location_coords, device, save_path=save_path)
    """ADD until here"""

    "DELETE below till print(Done)"
