...
**Purpose:** Extraction of Earth Observation (EO) Embedding ($Q$).
...
In this script, the inference pipeline is modified and redefined as a process for **extracting and saving the features (Latent Representation) acquired by the Prithvi-EO encoder**.

#### Processing Pipeline

1. **Spatial Patching via Sliding Window**
   Receives a huge satellite image (GeoTIFF) as input and divides it into non-overlapping patches (windows) of `224x224` pixels that the model can process. 

2. **Feature Extraction from Final Layer**
   In the `run_model` function, the `forward_features()` method of the frozen Prithvi-EO encoder is called.
   The **"output of the final layer"** (immediately after passing through all 24 blocks of the Transformer) is obtained as the most abstracted semantic information of the target patch.

3. **Saving Query ($Q$) (Saving Tensors)**
...
**Purpose:** Integrates variable-length Earth Observation (EO) embeddings divided into multiple patches to generate a Embedding ($Q$).
